...
from framework.hpp_lib import asp_hpp, param_overwrite
from framework.placeholder_length import find_length
from framework.detection import *
from framework.argument_parser import get_args
from framework.fuzzer import Fuzzer
from framework.utility import load_payload_file, abort
from framework.http_utility import HTTPHelper
from framework.param_source_detector import detect_accepted_sources
from framework.response_analyzer import analyze_responses, print_request, \
    print_response, analyze_chars, analyze_encoded_chars, \
    analyze_accepted_sources
from framework.placeholder_manager import PlaceholderManager
from framework.obfuscation_lib import unicode_urlencode, urlencode, \
...
class WafSploit:
    ...
    def is_detection_set(self, args):
        ...
        for name in det_functions:
            if args[name] is not None:
                return
        abort("You need to specify at least one Detection Function.")
    ...
    def init_methods(self, args):
        ...
        if methods:
            if self.sig + "all" + self.sig in methods:
                ...
                methods.extend(
                    load_payload_file("framework/payloads/HTTP/methods.txt"))
    ...
    def start(self, args):
        ...
        if args["mode"] == "length":
            ...
            if len(methods) > 1:
                abort("Only you need to specify only one Method")
        ...
        # Fuzzing mode
            ...
            if fuzzing_placeholders> 1:
                abort("Multiple fuzzing placeholder signatures found. "
                      "Only one fuzzing placeholder is supported.")
            elif fuzzing_placeholders == 0:
                abort("No fuzzing placeholder signatures found.")
            self.is_detection_set(args)
            payloads = []
            if args["payloads"]:
                for p_file in args["payloads"]:
            ...
            abort("Unknown bypassing mode.")


if __name__ == "__main__":
    wafsploit = WafSploit()
    wafsploit.start(get_args())